...
RSpec.describe 'As A Visitor', type: :feature do
  ...
  xit 'has a functional application link on /favorites where i can choose which pets it applies to' do
    visit "/pets/#{@elena.id}"
    click_on('Favorite!')
    ...
    expect(page).to eq("Application for #{@elena.name}")
    expect(page).to eq("Application for #{@raccoon.name}")
    expect(page).to eq("Application for #{@foxy.name}")
  end
end
